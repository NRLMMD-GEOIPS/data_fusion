...
def check_command_line_args(arglist, argdict):
    ...
    # All other --fuse_* arguments are optional - but if they are included for
    # one fusion dataset, they must be included for all.  If there is more than
    # one included, ensure the number of arguments passed matches the number
    # of fusion datasets.
    if (
        ...
        raise ValueError(
            "If --fuse_output_format passed for any fusion dataset, must be "
            "passed for all"
        )
    ...
    if (
        ...
        raise ValueError(
            "If --fuse_dataset_name passed for any fusion dataset, must be "
            "passed for all"
        )
    if (
        ...
        raise ValueError(
            "If --fuse_self_register_source passed for any fusion dataset, must be "
            "passed for all"
        )
    if (
        ...
        raise ValueError(
            "If --fuse_self_register_platform passed for any fusion dataset, must be "
            "passed for all"
        )
    if (
        ...
        raise ValueError(
            "If --fuse_self_register_dataset passed for any fusion dataset, must "
            "be passed for all"
        )
    if (
        ...
        raise ValueError(
            "If --fuse_sectored_read passed for any fusion dataset, must be "
            "passed for all"
        )
    if (
        ...
        raise ValueError(
            "If --fuse_resampled_read passed for any fusion dataset, must be "
            "passed for all"
        )
    ...
    if argdict["reader_name"]:
        raise ValueError(
            "--reader_name not supported, need to pass "
            "--fuse_files and --fuse_reader at the command line"
        )
    if argdict["product_name"]:
        raise ValueError(
            "--product_name not supported, need to pass "
            "--fusion_final_product_name at the command line"
        )
    if argdict["output_formatter"]:
        raise ValueError(
            "--output_formatter not supported, need to pass "
            "--fusion_final_output_formatter at the command line"
        )
...
def add_args(parser, arglist=None, legacy=False):
    ...
    if arglist is None or "fuse_files" in arglist:
        ...
        #     nargs="+",
        #     default=None,
        #     help="""Provide additional files required for fusion product. Files
        #             passed under this flag MUST be from the same source.
        #             fuse_files may be passed multiple times. Reader name for these
        #             files is specified with the fuse_reader flag.""",
        # )
        fusion_group.add_argument(
            "--fuse_reader_name",
            action="append",
            ...
            help="""Provide the reader name for files passed under the fuse_files flag.
                    Only provide one reader to this flag. If multiple fuse_files
                    flags are passed, the same number of fuse_readers must be
        ...
        fusion_group.add_argument(
            ...
            action="append",
            default=None,
            help="""Provide the product name for files passed under the fuse_files
                    flag. Only provide one product to this flag. If multiple
                    fuse_files flags are passed, the same number of fuse_products
        ...
        #     action="append",
        #     default=None,
        #     help="""Identify whether the reader specified for --fuse_files /
        #             --fuse_reader perform a resampled_read.  If not specified, a
        #             resampled read will NOT be performed.  If multiple fuse_files
        #             flags are passed, either the same number of fuse_resampled_read
        #             flags must be passed in the same order, or no fuse_resampled_read
        #             flags can be passed.""",
        # )
        # fusion_group.add_argument(
        #     "--fuse_sectored_read",
        ...
        #     default=None,
        #     help="""Identify whether the reader specified for --fuse_files /
        #             --fuse_reader perform a sectored_read.  If not specified, a
        #             sectored_read will NOT be performed.  If multiple fuse_files
        #             flags are passed, either the same number of fuse_sectored_read
        #             flags must be passed in the same order, or no fuse_sectored_read
        #             flags can be passed.""",
        # )
        # fusion_group.add_argument(
        ...
        #             fuse_files should be registered.
        #             If not specified, data will not be registered to a dataset,
        #             but the requested area_def. If multiple fuse_files flags
        #             are passed, either the same number of fuse_self_register_dataset
        #             flags must be passed in the same order, or no
        #             fuse_self_register_dataset flags can be passed.""",
        # )
        # fusion_group.add_argument(
        ...
        #             fuse_files should be registered.
        #             If not specified, data will not be registered to a dataset,
        #             but the requested area_def. If multiple fuse_files flags are
        #             passed, either the same number of fuse_self_register_source
        #             flags must be passed in the same order, or no
        #             fuse_self_register_source flags can be passed.""",
        # )
        # fusion_group.add_argument(
        ...
        fusion_group.add_argument(
            ...
            nargs="?",
            default=None,
            help="""Identify the name of the final product
                    (which includes all fusion datasets)
                                       """,
        )
        fusion_group.add_argument(
            ...
            nargs="?",
            default=None,
            help="""Identify the output_format for the final product
                    (using all fusion datasets)
                                       """,
        )
        fusion_group.add_argument(
            ...
            nargs="?",
            default=None,
            help="""Identify the source name that will be attributed to the
                    final dataset. This will be an identifier that encompasses
                    all datasets contained within the final product, and will be
                    referenced in "product_inputs" yamls
        ...
        fusion_group.add_argument(
            ...
            nargs="?",
            default=None,
            help="""Identify the platform name that will be attributed to the
                    final dataset. This will be an identifier that encompasses
                    all datasets contained within the final product, and will be
                    referenced in filenames, titles and metadata yamls
        ...
        fusion_group.add_argument(
            ...
            action="append",
            default=None,
            help="""If applicable, identify a unique dataset id (can be
                    numeric or string)
                                       """,
        )
