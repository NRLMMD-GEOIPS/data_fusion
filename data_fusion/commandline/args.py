...
def check_command_line_args(arglist, argdict):
    ...
    if not argdict['fuse_files'] or not argdict['fuse_reader_name']:
        raise ValueError('Need to pass --fuse_files and --fuse_reader_name at the command line')
    ...
    if len(argdict['fuse_reader_name']) != num_fusion_datasets:
        raise ValueError('Number of passed readers does not match the number of fusion file groups')
    if len(argdict['fuse_product_name']) != num_fusion_datasets:
        raise ValueError('Number of passed products does not match the number of fusion file groups')
    ...
    if argdict['fuse_output_format'] and len(argdict['fuse_output_format']) != num_fusion_datasets:
        raise ValueError('If --fuse_output_format passed for any fusion dataset, must be passed for all')
    if argdict['fuse_order'] and len(argdict['fuse_order']) != num_fusion_datasets:
        raise ValueError('If --fuse_order passed for any fusion dataset, must be passed for all')
    if argdict['fuse_dataset_name'] and len(argdict['fuse_dataset_name']) != num_fusion_datasets:
        raise ValueError('If --fuse_dataset_name passed for any fusion dataset, must be passed for all')
    if argdict['fuse_self_register_source'] and len(argdict['fuse_self_register_source']) != num_fusion_datasets:
        raise ValueError('If --fuse_self_register_source passed for any fusion dataset, must be passed for all')
    if argdict['fuse_self_register_platform'] and len(argdict['fuse_self_register_platform']) != num_fusion_datasets:
        raise ValueError('If --fuse_self_register_platform passed for any fusion dataset, must be passed for all')
    if argdict['fuse_self_register_dataset'] and len(argdict['fuse_self_register_dataset']) != num_fusion_datasets:
        raise ValueError('If --fuse_self_register_dataset passed for any fusion dataset, must be passed for all')
    if argdict['fuse_sectored_read'] and len(argdict['fuse_sectored_read']) != num_fusion_datasets:
        raise ValueError('If --fuse_sectored_read passed for any fusion dataset, must be passed for all')
    if argdict['fuse_resampled_read'] and len(argdict['fuse_resampled_read']) != num_fusion_datasets:
        raise ValueError('If --fuse_resampled_read passed for any fusion dataset, must be passed for all')
...
def get_command_line_args(arglist=None, description=None):
    ...
    '''
    return geoips_get_args(arglist, description, add_args_func=add_args, check_args_func=check_command_line_args)
...
def add_args(parser, arglist=None):
    ...
    if arglist is None or 'fuse_files' in arglist:
        ...
        fusion_group.add_argument('--fuse_reader_name', action='append', default=None,
        ...
        fusion_group.add_argument('--fuse_product_name', action='append', default=None,
        ...
        fusion_group.add_argument('--fusion_final_source_name', nargs='?', default=None,
                                  help='''Identify the source name that will be attributed to the final dataset.
                                       ...
                                       ''')
        fusion_group.add_argument('--fusion_final_platform_name', nargs='?', default=None,
                                  help='''Identify the platform name that will be attributed to the final dataset.
                                       ...
                                       ''')
        fusion_group.add_argument('--fuse_output_format', action='append', default=None,
                                  help='''If applicable, identify the output format module to be used with
                                       the given product.''')
        fusion_group.add_argument('--fuse_order', action='append', default=None,
                                  help='''If applicable, identify the order each fused product should be
                                       ...
                                       must all be unique).''')
        fusion_group.add_argument('--fuse_dataset_name', action='append', default=None,
