...
from geoips.plugins.modules.procflows.single_source import pad_area_definition, get_alg_xarray
...
from geoips.plugins.modules.procflows.single_source import get_area_defs_from_command_line_args
...
def get_overall_start_datetime(fuse_dict):
    ...
    for fuse_name in fuse_dict:
        if 'metadata_xobj' in fuse_dict[fuse_name] \
           and 'start_datetime' in fuse_dict[fuse_name]['metadata_xobj'].attrs \
           and ( min_start_datetime is None \
           or fuse_dict[fuse_name]['metadata_xobj'].attrs['start_datetime'] < min_start_datetime):
            min_start_datetime = fuse_dict[fuse_name]['metadata_xobj'].attrs['start_datetime']
...
def get_overall_end_datetime(fuse_dict):
    ...
    for fuse_name in fuse_dict:
        if 'metadata_xobj' in fuse_dict[fuse_name] \
           ...
           and ( max_end_datetime is None \
           or fuse_dict[fuse_name]['metadata_xobj'].attrs['end_datetime'] > max_end_datetime):
            max_end_datetime = fuse_dict[fuse_name]['metadata_xobj'].attrs['end_datetime']
...
def unpack_fusion_arguments(argdict):
    ...
    for fusion_file_list in fusion_files:
        ...
        curr_fuse_dict = {'reader_name': fusion_reader_name,
        ...
        if fusion_self_register_sources:
            curr_fuse_dict['self_register_source'] = fusion_self_register_sources[curr_num]
        if fusion_self_register_platforms:
            curr_fuse_dict['self_register_platform'] = fusion_self_register_platforms[curr_num]
        if fusion_self_register_datasets:
            curr_fuse_dict['self_register_dataset'] = fusion_self_register_datasets[curr_num]
    ...
    requested_fusion['final'] = {'product_name': argdict['fusion_final_product_name'],
    ...
    final_metadata_xobj.attrs['product_name'] = requested_fusion['final']['product_name']
    final_metadata_xobj.attrs['platform_name'] = requested_fusion['final']['platform_name']
    final_metadata_xobj.attrs['output_format'] = requested_fusion['final']['output_format']
    final_metadata_xobj.attrs['start_datetime'] = requested_fusion['final']['start_datetime']
    final_metadata_xobj.attrs['end_datetime'] = requested_fusion['final']['end_datetime']
...
def get_fused_xarray(area_def, fuse_data):
    ...
    for fuse_data_name in fuse_data:
        ...
        try:
            ...
            if 'variables' in final_product_for_curr_source:
                required_variables += list(set(get_required_variables(final_product_name, source_name)))
        except KeyError:
            LOG.info('No additional requirements for source %s / final product %s', source_name, final_product_name)
        ...
        if product_type != 'unmodified' and 'pad_area_definition' in product and product['pad_area_definition']:
            pad_area_def = pad_area_definition(area_def, force_pad=True, x_scale_factor=1.5, y_scale_factor=1.5)
        else:
        ...
        reader_out = reader(files, metadata_only=False, chans=required_variables, area_def=pad_area_def)

        if product_type != 'unmodified':
            pad_sect_xarrays = sector_xarrays(reader_out, pad_area_def, required_variables)
        else:
        ...
        alg_product_types = ['alg', 'interp_alg', 'interp_alg_cmap', 'alg_interp_cmap', 'alg_cmap', 'cmap']
        ...
        if product_type in alg_product_types+interp_only_product_types:
            interp_xarrays[dataset_name] = get_alg_xarray(pad_sect_xarrays,
                                                          area_def,
                                                          product_name,
            ...
            for attrname in fuse_data[dataset_name].keys():
                if attrname == 'metadata_xobj':
                    continue
                interp_xarrays[dataset_name].attrs[attrname] = fuse_data[dataset_name][attrname]
        # If no modification required to data, just set pad_sect_xarrays to interp_xarrays[dataset_name]
            ...
            interp_xarrays[dataset_name]['METADATA'].attrs['product_definition'] = product
            # Set all the fuse_data attrs on the METADATA xarray obj
            for attrname in fuse_data[dataset_name].keys():
                if attrname == 'metadata_xobj':
                    continue
                interp_xarrays[dataset_name]['METADATA'].attrs[attrname] = fuse_data[dataset_name][attrname]
        else:
            raise ValueError('Product type {product_type} must be one of {alg_product_type}, {interp_only_product_type}, or {noalg_product_type}')
        if 'coverage_dataset' in final_product and final_product['coverage_dataset'] == dataset_name:
    ...
    if coverage_dataset:
        ...
        interp_xarrays['METADATA'].attrs['start_datetime'] = coverage_dataset.attrs['start_datetime']
        interp_xarrays['METADATA'].attrs['end_datetime'] = coverage_dataset.attrs['end_datetime']
    ...
    no_alg_product_types = ['sectored_xarray_dict_to_output_format',
                            'unsectored_xarray_dict_to_output_format',
                            'xarray_dict_to_output_format']
...
def run_fuse_alg(fuse_xarrays, fuse_product_name, fuse_source_name):
    ...
    if alg_func_type in ['xarray_dict_to_xarray', 'xarray_dict_to_xarray_dict']:
        ...
        raise TypeError(f'Unsupported algorithm type for fusion driver: {alg_func_type}')
...
def data_fusion(fnames, command_line_args=None):
    ...
    check_args = ['sectorlist', 'sectorfiles',  # Static sectors,
                  'tcdb', 'tcdb_sectorlist',  # TC Database sectors,
                  'trackfiles', 'trackfile_parser', 'trackfile_sectorlist',  # Flat text trackfile,
                  'fuse_files', 'fuse_reader_name', 'fuse_product_name']
    ...
    area_defs = get_area_defs_from_command_line_args(command_line_args,
    ...
    for area_def in area_defs:
        ...
        if fused_xarray is not None:
            ...
            output_format_kwargs = get_output_formatter_kwargs(command_line_args,
            ...
            LOG.info('SKIPPING No coverage or required variables for %s %s', alg_xarray.source_names, area_def.name)
    ...
    if compare_path:
        retval = compare_outputs(compare_path.replace('<product>', final_product_name).\
                                                  replace('<procflow>', 'data_fusion'),
                                 final_products)
